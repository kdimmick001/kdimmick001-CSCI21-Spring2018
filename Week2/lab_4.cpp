...
char StringToChar(string value) {
  ...
  if ((value.at(0) == value.size(0)) || (value.at(0) > 1)) {
...
double StringToDouble(string value) {
 ...
 for (int i = 0; i < value.size(); ++i){
     if ((value.at(i) >= 48) || (value.at(i) <= 57)) {
         number = static_cast<double>(value.at(i));
     }
 }
...
bool StringToBool(string value) {
  ...
  for (int i = 0; i < value.size(); ++i){
    if ((value.at(i) == 84) || (value.at(i) ==70)){
      question = static_cast<bool>(value.at(i));
    }
  }
